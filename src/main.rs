use chrono::{DateTime, Datelike, FixedOffset, LocalResult, TimeZone, Utc, Timelike};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // 保存分析 JSON
    let summary_file = File::create("processed_summary.json")?;
    serde_json::to_writer_pretty(summary_file, &final_report)?;

    println!("✅ 分析完成！结果已写入 processed_summary.json");
    Ok(())
}
...
fn perform_final_analysis(reports: &[CommitReport]) -> FinalAnalysisReport {
    ...
    for r in reports {
        ...
        *daily_commits.entry(dt.format("%Y-%m-%d").to_string()).or_insert(0) += 1;
        hourly_dist[dt.hour() as usize] += 1;
    ...
    for (email, (info, commits)) in author_map {
        ...
        let active_days: HashSet<String> = commits.iter().map(|c| {
            Utc.timestamp_opt(c.author.timestamp, 0).unwrap().format("%Y-%m-%d").to_string()
        }).collect();

        // 简易公式: Impact = Commits*0.3 + log(Ins)*0.5 + Days*0.2
    ...
    FinalAnalysisReport {
        ...
        engineering_quality: EngineeringQuality {
            ...
            avg_files_per_commit: if reports.len() > 0 { reports.iter().map(|r| r.files_changed_count).sum::<usize>() as f64 / reports.len() as f64 } else { 0.0 },
            gpg_signed_percentage: if reports.len() > 0 { (gpg_count as f64 / reports.len() as f64 * 100.0) } else { 0.0 },
            potential_secrets_found: 0,
        },
    }
}

// analyze_commit 函数（完全保留用户源码）
fn analyze_commit(repo: &Repository, commit: &Commit) -> Result<CommitReport, Box<dyn Error>> {
    ...
    Ok(CommitReport {
        ...
        parent_hashes, author, committer, summary: commit.summary().unwrap_or("").to_string(),
        body: commit.body().map(|s| s.to_string()),
        is_merge: commit.parent_count() > 1,
        signature_verified, total_insertions: stats.insertions(),
        total_deletions: stats.deletions(), files_changed_count: stats.files_changed(),
        changes,
    })
}
